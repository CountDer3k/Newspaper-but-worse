...
public class RESTTest {
	...
	@Test
	public void testCreateArticle_exception() {
		PostModel pm = makePost();
		pm.setId(1);
		ResponseEntity<Object> expected = ResponseEntity.ok("Internal error occured");
		PostDto dto = new PostDto();

		when(service.addNewPost(any(PostDto.class), anyInt())).thenReturn(pm);

		ResponseEntity<Object> actual = controller.createArticle("abc", any(String.class), any(String.class));
	...
	public void testGetPost() {
		PostModel p = makePost();
		ResponseEntity<PostModel> expected = ResponseEntity.ok(p);

		when(service.getPostByID("1")).thenReturn(p);

		ResponseEntity<Object> actual = controller.getPost("1");
		Assert.assertEquals(expected, actual);
	}

	@Test
	public void testDeletePost_true() {
		ResponseEntity<Boolean> expected = ResponseEntity.ok(true);

		when(service.deletePost("1")).thenReturn(true);

		ResponseEntity<Boolean> actual = controller.deletePost("1");
		Assert.assertEquals(expected, actual);
	}
	
	@Test
	public void testDeletePost_false() {
		ResponseEntity<Boolean> expected = ResponseEntity.ok(false);

		when(service.deletePost("1")).thenReturn(false);
	...
	public void testGetPost_null() {
		...
		ResponseEntity<Object> actual = controller.getPost(any(String.class));
		Assert.assertEquals(expected, actual);
	}
